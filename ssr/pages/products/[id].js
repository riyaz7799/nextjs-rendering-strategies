...
export default function SSRProductDetail({ product }) {
  const [cartCount, setCartCount] = useState(0);
  const [added, setAdded] = useState(false);
  const [activeImg, setActiveImg] = useState(0);

  if (!product) return (
    <Layout><Header badge="ssr" /><div className="error-box"><h2>Product not found</h2></div></Layout>
  );
  ...
  return (
    <Layout title={`${product.title} — NexMart SSR`}>
      <Header cartCount={cartCount} badge="ssr" />
...
const styles = {
  ...
  catLabel: { fontSize:11,fontWeight:700,letterSpacing:'0.1em',textTransform:'uppercase',color:'#60a5fa' },
  ...
  detailPrice: { fontFamily:"'Playfair Display',serif",fontSize:36,fontWeight:900,color:'#60a5fa' },
  discountTag: { background:'rgba(96,165,250,0.15)',color:'#60a5fa',border:'1px solid rgba(96,165,250,0.3)',padding:'4px 10px',borderRadius:999,fontSize:12,fontWeight:700 },
  detailBtn: { padding:'14px 24px',background:'#60a5fa',color:'#000',border:'none',borderRadius:'var(--radius-sm)',fontWeight:700,fontSize:15,cursor:'pointer',transition:'var(--transition)' },
